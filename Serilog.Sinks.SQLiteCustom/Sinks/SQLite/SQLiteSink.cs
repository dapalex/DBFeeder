...
namespace Serilog.Sinks.SQLite
{
    internal class SQLiteSink : BatchProvider, ILogEventSink
    {
        ...
        private void CreateSqlTable(SQLiteConnection sqlConnection)
        {
            ...
            colDefs += "SERVICE TEXT,";
            colDefs += "SOURCE TEXT,";
            colDefs += "AMQPMESSAGE TEXT,";
            colDefs += "LOGMESSAGE TEXT,";
            colDefs += "Exception TEXT,";
            colDefs += "DT_ERROR TEXT";
        ...
        {
            var sqlInsertText = "INSERT INTO {0} (SERVICE, SOURCE, AMQPMESSAGE, LOGMESSAGE, Exception, DT_ERROR)";
            sqlInsertText += " VALUES (@SERVICE, @SOURCE, @AMQPMESSAGE, @LOGMESSAGE, @exception, @DT_ERROR)";
            ...
            sqlCommand.Parameters.Add(new SQLiteParameter("@SERVICE", DbType.String));
            sqlCommand.Parameters.Add(new SQLiteParameter("@SOURCE", DbType.String));
            sqlCommand.Parameters.Add(new SQLiteParameter("@AMQPMESSAGE", DbType.String));
            sqlCommand.Parameters.Add(new SQLiteParameter("@LOGMESSAGE", DbType.String));
            sqlCommand.Parameters.Add(new SQLiteParameter("@exception", DbType.String));
            sqlCommand.Parameters.Add(new SQLiteParameter("@DT_ERROR", DbType.DateTime2));
        ...
        private async Task WriteToDatabaseAsync(ICollection<LogEvent> logEventsBatch, SQLiteConnection sqlConnection)
        {
            using (var tr = sqlConnection.BeginTransaction())
            {
                using (var sqlCommand = CreateSqlInsertCommand(sqlConnection))
                {
                    ...
                    {
                        sqlCommand.Parameters["@SERVICE"].Value = logEvent.Properties["SourceContext"];
                        sqlCommand.Parameters["@SOURCE"].Value = logEvent.Properties["ThreadName"];
                        sqlCommand.Parameters["@AMQPMESSAGE"].Value = logEvent.Properties["MESSAGE"];
                        sqlCommand.Parameters["@LOGMESSAGE"].Value = logEvent.RenderMessage();
                        sqlCommand.Parameters["@exception"].Value =
                            logEvent.Exception?.ToString() ?? string.Empty;
                        sqlCommand.Parameters["@DT_ERROR"].Value = _storeTimestampInUtc
